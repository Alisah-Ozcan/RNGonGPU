...
namespace rngongpu
{
    ...
    // The design follows the structure described in NIST SP 800‑90A Section 10.2.1:
    // "CTR_DRBG Instantiation" uses a derivation function (DF) that takes an input string
    // of the form: [requestedOutputBits (4 bytes) || inputLengthBits (4 bytes) || input || 0x80 || padding].
    // The DRBG uses AES in counter mode (with an internal state of key and V) and supports
    // prediction resistance and reseed as described in the standard.
    class AESCTRRNG {
    public:
        // Constructor (Instantiation)
        // entropyInput: Entropy input. Recommended length is equal to the security strength:
        //   AES-128: 16 bytes, AES-192: 24 bytes, AES-256: 32 bytes.
        // nonce: A nonce value (we choose the same length as entropy for higher security).
        // personalizationString: Optional string; can be empty.
        // secLevel: Security level (AES128, AES192, or AES256).
        // predictionResistance: If true, reseed is performed on each getRandomBytes() call.
        ...
        // reseed: Reseeds the DRBG with new entropy and optional additional input.
        // If additionalInput is provided, its length must be exactly seedLen (keyLen + 16 bytes).
        void reseed(const std::vector<unsigned char>& additionalInput = std::vector<unsigned char>());
    ...
    private:
        ...
        // Derived parameters:
        std::size_t keyLen;  // Key length in bytes (16 for AES-128, 24 for AES-192, 32 for AES-256)
        ...
        // CTR_DRBG_Update: Updates the internal state (key and V) using the provided data.
        // If providedData is empty, it is treated as a zero string of length seedLen.
        ...
        void validateEntropyInput(const std::vector<unsigned char>& entropyInput);
        void validateAdditionalInput(const std::vector<unsigned char>& additionalInput);

        // Helper functions to select the appropriate EVP_CIPHER for the chosen security level.
